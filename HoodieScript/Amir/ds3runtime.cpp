	...
	while (true) {
		auto itr = std::find_if(scripts.begin(), scripts.end(), [](auto&& script) {
			...
			if (detachResult) script->remove();
				...
				logging::write_line("Script {} was unable to detach...", script->getName());
				script->setDetaching();
			}
...
void DS3RuntimeScripting::addHook(std::unique_ptr<jump_hook> hook)
{
...
jump_hook* DS3RuntimeScripting::accessHook(const std::string& name)
{
	jump_hook* matchingHook = nullptr;
