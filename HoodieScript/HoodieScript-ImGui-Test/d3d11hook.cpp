...
#include "InputHook.h"
//#include <HoodieScript-ImGui-Test/imgui_impl_dx11.cpp>
...
HRESULT __stdcall PresentHook(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	std::call_once(g_isInitialized, [&]() {
		...
		ImGui_ImplDX11_Init(g_hWnd, g_pd3dDevice, g_pd3dContext, pSwapChain != NULL ? pSwapChain : g_pSwapChain);
		inputHook.Init(g_hWnd);
	});

	ImplHookDX11_Present(g_pd3dDevice, g_pd3dContext, pSwapChain != NULL ? pSwapChain : g_pSwapChain);
...
DWORD __stdcall HookDX11_Init()
{
	...
	if (FAILED(hr))
	{
		//hoodie_script::logging::write_line("Failed to create D3D device and swapchain");
		return E_FAIL;
	}

	//hoodie_script::logging::write_line("Correctly created D3D Device");
	...
	//hoodie_script::logging::write_line("D3D hooks installed\r\n");
...
typedef HRESULT(WINAPI *D3D11CREATEDEVICEANDSWAPCHAIN)(IDXGIAdapter*, D3D_DRIVER_TYPE, HMODULE, UINT, D3D_FEATURE_LEVEL*, UINT, UINT, DXGI_SWAP_CHAIN_DESC*, IDXGISwapChain**, ID3D11Device**, D3D_FEATURE_LEVEL*, ID3D11DeviceContext**);
D3D11CREATEDEVICEANDSWAPCHAIN fpD3D11CreateDeviceAndSwapChain = NULL;
HRESULT WINAPI tD3D11CreateDeviceAndSwapChain(IDXGIAdapter* pAdapter, D3D_DRIVER_TYPE DriverType, HMODULE Software, UINT Flags, const D3D_FEATURE_LEVEL* pFeatureLevels, UINT FeatureLevels, UINT SDKVersion, DXGI_SWAP_CHAIN_DESC* pSwapChainDesc, IDXGISwapChain** ppSwapChain, ID3D11Device** ppDevice, D3D_FEATURE_LEVEL* pFeatureLevel, ID3D11DeviceContext** ppImmediateContext)
{
	wprintf(L"[ModEngine] D3D Device create\r\n");
	HRESULT hr = fpD3D11CreateDeviceAndSwapChain(pAdapter, DriverType, Software, Flags, pFeatureLevel, FeatureLevels, SDKVersion, pSwapChainDesc, ppSwapChain, ppDevice, pFeatureLevel, ppImmediateContext);
	if (FAILED(hr))
	{
	...
	g_pd3dContext = *ppImmediateContext;
...
typedef HRESULT(WINAPI* D3D11CREATEDEVICE)(IDXGIAdapter*, D3D_DRIVER_TYPE, HMODULE, UINT, D3D_FEATURE_LEVEL*, UINT, UINT, ID3D11Device**, D3D_FEATURE_LEVEL*, ID3D11DeviceContext**);
D3D11CREATEDEVICE fpD3D11CreateDevice = NULL;
HRESULT WINAPI tD3D11CreateDevice(IDXGIAdapter* pAdapter, D3D_DRIVER_TYPE DriverType, HMODULE Software, UINT Flags, const D3D_FEATURE_LEVEL* pFeatureLevels, UINT FeatureLevels, UINT SDKVersion, ID3D11Device** ppDevice, D3D_FEATURE_LEVEL* pFeatureLevel, ID3D11DeviceContext** ppImmediateContext)
{
	/*wprintf(L"[ModEngine] D3D Device create\r\n");
	HRESULT hr = fpD3D11CreateDevice(pAdapter, DriverType, Software, Flags, pFeatureLevel, FeatureLevels, SDKVersion, ppDevice, pFeatureLevel, ppImmediateContext);
	if (FAILED(hr))
	{
		//MessageBoxW(g_hWnd, L"Failed to create device and swapchain.", L"Fatal Error", MB_ICONERROR);
		wprintf(L"[ModEngine] Failed to create device and swapchain.\r\n");
		return hr;
	}*/
	return tD3D11CreateDeviceAndSwapChain(pAdapter, DriverType, Software, Flags, pFeatureLevels, FeatureLevels, SDKVersion, nullptr, nullptr, ppDevice, pFeatureLevel, ppImmediateContext);
	/*g_pSwapChain = *ppSwapChain;
	g_pd3dDevice = *ppDevice;
	g_pd3dContext = *ppImmediateContext;
