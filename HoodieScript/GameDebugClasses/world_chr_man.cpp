...
namespace hoodie_script {
	...
	std::vector<uintptr_t> WorldChrMan::getCurrentMapEnemies()
	{
		...
		if (hasInstance())
		{
			auto aPa = accessMultilevelPointer<unsigned int>(getInstance() + 0x1d0, 0x0);
			if (aPa != nullptr)
			{
				auto aP = *aPa;
				for (size_t i = 0; i < aP; i++)
				{
					auto a = accessMultilevelPointer<uintptr_t*>(getInstance() + 0x1d0, 0x8);
					if (a != nullptr)
					{
	...
	bool WorldChrMan::reloadCharacterFiles(std::wstring characterId)
	{
		if (hasInstance())
		{
			...
			*(uintptr_t*)&ReloadChrFunction = 0x1408D1E10;
			ReloadChrFunction(getInstance(), characterId.c_str());
			return true;
		}
		return false;
	}
	...
	std::array<float, 3> WorldChrMan::getCamVector()
	{
		...
		if (hasInstance())
		{
			memcpy(&quaternion[0], accessMultilevelPointer<float>(getInstance() + 0x31A0, 0x30), sizeof(float) * 3);
		}
		return quaternion;
	}
